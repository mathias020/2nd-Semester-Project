...
public class ClientController implements Controller {
   ...
   public ClientController(MovieCornerModel model, MovieCornerView view) {
      this.model = model;
      this.view = view;
      user = null;
   }
   ...
   private void getFriends() {
      ...
      if(friendIDs != null)
         for (int i = 0; i < friendIDs.length; i++)
            friends.add(model.getUserById(friendIDs[i]));
      
      view.show(prepareList(friends.toArray(), "getFriends"));
   }
   ...
   private void saveChanges() {
      ...
      MyDate theDate = item.getReleaseDate();
      
      if(theDate.getDay() > theDate.daysInMonth() || theDate.getDay() < 1)
      {
         view.show("Please use the correct date format (eg. 03/12/2015)");
         return;
      }
      
      if(theDate.getMonth() > 12 || theDate.getMonth() < 1)
      {
         view.show("Please use the correct date format (eg. 03/12/2015)");
         return;
      }
   ...
   private void addEpisode() {
      ...
      MyDate theDate = tempEp.getReleaseDate();
